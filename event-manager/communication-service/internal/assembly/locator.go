...
import (
	...
	"github.com/PabloPerdolie/event-manager/communication-service/pkg/rabbimq"
	_ "github.com/lib/pq"
...
func NewLocator(cfg *config.Config, logger *zap.SugaredLogger) (*ServiceLocator, error) {
	...
	commentConsumer := rabbimq.New(commentHandler, cfg, logger)

	go func() {
		if err := commentConsumer.Start(); err != nil {
			logger.Fatalw("Failed to start RabbitMQ rabbimq", "error", err)
		}
	}()
